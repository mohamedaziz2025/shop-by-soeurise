...
export const useCartStore = create<CartState>((set, get) => ({
  ...
  fetchCart: async () => {
    ...
    try {
      ...
      const cartData = await api.getCart(guestId || undefined);
      set({
        cart: cartData,
  ...
  addItem: async (data) => {
    try {
      // Ensure guestId exists and include it for anonymous users
      let guestId: string | null = null;
      if (typeof window !== 'undefined') {
        guestId = localStorage.getItem('guestId');
        if (!guestId) {
      ...
      await api.addToCart({ ...data, guestId });
      await get().fetchCart();
    } catch (error) {
  ...
  updateItemQuantity: async (productId, variantId, quantity) => {
    try {
      const guestId = typeof window !== 'undefined' ? localStorage.getItem('guestId') : undefined;
      await api.updateCartItem(productId, variantId, quantity, guestId || undefined);
      await get().fetchCart();
    } catch (error) {
  ...
  removeItem: async (productId, variantId) => {
    try {
      const guestId = typeof window !== 'undefined' ? localStorage.getItem('guestId') : undefined;
      await api.removeFromCart(productId, variantId, guestId || undefined);
      await get().fetchCart();
    } catch (error) {
